...
> ⚠️ **BGM 관련 미해결 위험** — 6번에서 바꾼 것은 `files[0].url` 하나뿐입니다. 같은 노드의
> `dashAudioFiles[0]`, `hlsManifestUrl`, `dashVideoFiles`는 **여전히 원본 음원**(`_assets/video/…`)을
> 가리킵니다. Chrome 네트워크 캡처에서는 이 대체 경로가 요청되지 않는 것을 확인했지만,
> **Safari는 HLS를 네이티브로 선호하는 브라우저라 확인 대상에서 빠져 있습니다.**
> 만약 Safari가 HLS 경로를 타면 **아이폰에서만 원본 음악이 재생**됩니다.
> 하객 상당수가 아이폰 + 카카오톡 인앱 브라우저이므로, 배포 전에 아이폰에서 직접 들어보고
> 확인해야 합니다. 옛 음악이 나온다면 대체 URL까지 정리해야 합니다.
...
**JS 모듈을 손볼 때 반드시 알아야 할 것 두 가지** — 어겼을 때 실제로 깨졌던 항목입니다.

- Canva 런타임은 화면 크기가 바뀌면 관련 inline style을 **통째로 다시 씁니다.** 그래서 세 모듈 모두
  `MutationObserver`로 상시 재적용합니다. 1회성으로 적용하면 리사이즈 순간 사라집니다.
